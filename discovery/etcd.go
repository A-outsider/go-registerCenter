...
//	closeChan     chan struct{}                           // 手动关闭的管道
//}
//
//// 使用config中的信息将GRPC服务注册成ETCD节点
//func RegisterEtcdEndpoint(name, addr, version string) {
//	// 创建etcd对象
//	etcd, err := NewEtcd(config.Get().Etcd.Addr, int64(config.Get().Etcd.Ttl), &Server{
//		Name:    name,
//		Addr:    addr,
//		Version: version,
//	})
...
//
//	// 创建一个节点管理对象em
//	em, err := endpoints.NewManager(e.cli, e.srvInfo.target()) // 服务名称
//	if err != nil {
//		return
//	}
//
//	// 为当前服务创建节点
//	err = em.AddEndpoint(context.TODO(), e.srvInfo.key(), endpoints.Endpoint{Addr: e.srvInfo.Addr}, clientv3.WithLease(lease.ID)) //这里服务名称 + 服务地址 做为服务唯一key
//	if err != nil {
//		return
//	}
//
//	// 启动自动续约
//	e.keepAliveChan, err = e.cli.KeepAlive(context.TODO(), lease.ID) //持续不断地向 etcd 服务器发送保活请求
//	if err != nil {
//		return
//	}
//
